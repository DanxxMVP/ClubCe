...
// ---- NUEVA FUNCIÓN: Repulsión entre células del mismo jugador ----
function applyCellRepulsion(cells, minDistance = 20) {
    for (let i = 0; i < cells.length; i++) {
        for (let j = i + 1; j < cells.length; j++) {
            const dx = cells[j].x - cells[i].x;  // Corregido: usa "-" en lugar de "="
            const dy = cells[j].y - cells[i].y;  // Corregido: usa "-" en lugar de "="
            const distance = Math.sqrt(dx * dx + dy * dy);
            
            // Distancia mínima requerida (radios + espacio extra)
            const requiredDistance = cells[i].radius + cells[j].radius + minDistance;  // Corregido: usa "+" en lugar de "="
            
            if (distance < requiredDistance) {
                // Fuerza de repulsión (ajusta el 0.25 para más/menos separación)
                const force = (requiredDistance - distance) / requiredDistance * 0.25;  // Corregido: usa "-" en lugar de "="
                cells[i].x -= dx * force * 0.5;  // Corregido: usa "-="
                cells[i].y -= dy * force * 0.5;   // Corregido: usa "-="
                cells[j].x += dx * force * 0.5;   // Corregido: usa "+="
                cells[j].y += dy * force * 0.5;   // Corregido: usa "+="
            }
        }
    }
}
...
const tickPlayer = currentPlayer => {
    // Aplica repulsión entre células del mismo jugador (si tiene más de 1 célula)
    if (currentPlayer.cells.length > 1) {
        applyCellRepulsion(currentPlayer.cells, 20); // 20 = espacio extra (pixeles)
    }
...
const gameloop = () => {
    if (map.players.data.length > 0) {
        ...
        // Reduce la velocidad de fusión (cambia 0.1 a un valor más bajo si es necesario)
        map.players.shrinkCells(0.5,fig.defaultPlayerMass, config.minMassLoss);
    }
